...
# MARKET_DATA_PATH = "../second_round_datasets/second_round_train_market_data.csv"
# FUNDAMENTAL_DATA_PATH = "../second_round_datasets/second_round_train_fundamental_data.csv"
# RETURN_DATA_PATH = "../second_round_datasets/second_round_train_return_data.csv"
###################################################################################################################
...
###################################################################################################################
    ...
    unique_investment_codes = []
    for each_investment in all_X_dict:
        selected_X = copy.deepcopy(all_X_dict[each_investment])
        selected_X["investment_code"] = selected_X.index.map(lambda x: int(x.split("d")[0][1:]))
        unique_investment_codes.extend(selected_X["investment_code"].unique())
        selected_X = selected_X.reset_index(drop=True)

    unique_investment_codes = sorted(list(set(unique_investment_codes)))
    for each_investment in all_X_dict:
        selected_X = copy.deepcopy(all_X_dict[each_investment])
        selected_X["investment_code"] = selected_X.index.map(lambda x: int(x.split("d")[0][1:]))
        selected_X["investment_code"] = pd.Categorical(selected_X["investment_code"], categories=unique_investment_codes)
        selected_X = selected_X.reset_index(drop=True)
        if selected_X.dropna().shape[0] != 0:
            predicted_value_series[each_investment] = model_storage_object["model"].predict(selected_X)[0]
...
def train_model():
    ...
    # Add this function to scale the features
    ...
    for each_investment in model_complete_df["investment_code"].unique():
        ...
        model_train_df = pd.concat([model_train_df, sample_train_df.reset_index(drop=True)])
        # model_validation_df = model_validation_df.append(temp_selected_df.drop(sample_train_df.index), ignore_index = True)
        model_validation_df = pd.concat([model_validation_df, temp_selected_df.drop(sample_train_df.index).reset_index(drop=True)])
    ...
    validation_X = scale_features(validation_X)
    ...
    def lightgbm_factory(argsDict, train_X_fold, train_Y_fold, validation_X_fold, validation_Y_fold):
        argsDict = lgb_argsDict_tranform(argsDict)
        params = {"nthread": -1,  
                "max_depth": argsDict["max_depth"],
                "num_trees": argsDict["num_trees"],
        ...
        params["metric"] = ["mape"]
        train_data_lgb_fold = lgb.Dataset(data=train_X_fold, label=train_Y_fold, categorical_feature=["investment_code"])
        validation_data_lgb_fold = lgb.Dataset(data=validation_X_fold, label=validation_Y_fold, categorical_feature=["investment_code"])
        model_lgb = lgb.train(params=params, train_set=train_data_lgb_fold, num_boost_round=300, valid_sets=[validation_data_lgb_fold], early_stopping_rounds=100, categorical_feature=["investment_code"])
        return lgb_get_model_score_mape(model_lgb)
    ...
    #     validation_X_fold = train_X.iloc[validation_index]
    #     validation_Y_fold = train_Y.iloc[validation_index]
    #     lgb_best = fmin(fn=lambda args: lightgbm_factory(args, train_X_fold, train_Y_fold, validation_X_fold, validation_Y_fold),
    #                     space=lgb_space, algo=algo, max_evals=20, pass_expr_memo_ctrl=None)
    #     best_score_fold = lightgbm_factory(lgb_best, train_X_fold, train_Y_fold, validation_X_fold, validation_Y_fold)
    #     lgb_best = best_score_fold
    #     if lgb_best < best_score:
    #         best_score = lgb_best
    #         best_params = lgb_best
    # opt_res = lgb_argsDict_tranform(best_params)
    ...
    for train_index, validation_index in kfold.split(train_X):
        ...
        lgb_best = fmin(fn=lambda args: lightgbm_factory(args, train_X_fold, train_Y_fold, validation_X_fold, validation_Y_fold),
                        space=lgb_space, algo=algo, max_evals=20, pass_expr_memo_ctrl=None)
        # Get the best score from the model
        best_score_fold = lightgbm_factory(lgb_best, train_X_fold, train_Y_fold, validation_X_fold, validation_Y_fold)
